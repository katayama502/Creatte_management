    ...
    <div className="flex flex-col h-full min-h-screen bg-white">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 shrink-0">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center', colorBg)}>
            <Building className="w-4 h-4 text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-bold text-gray-900 truncate">{facility.name}</h1>
            <div className="flex items-center gap-3 text-xs text-gray-400">
              ...
              {facility.description && <span className="truncate">{facility.description}</span>}
            </div>
          </div>
          <button
            onClick={() => openNewReservation(format(currentDate, 'yyyy-MM-dd'))}
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm shrink-0"
          >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors font-medium"
            >
              今日
            </button>
            ...
            <h2 className="text-base font-semibold text-gray-900 min-w-[160px]">{getHeaderLabel()}</h2>
          </div>
